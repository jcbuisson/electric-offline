...
const pool = new Pool({ connectionString: process.env.DATABASE_URL || 'postgresql://localhost/todoDB' })
...
app.post('/api/todos', async (request, response, next) => {
   try {
      ...
      const { rows } = await pool.query(
         'INSERT INTO todo (label, completed) VALUES ($1, $2) RETURNING *', [label, Boolean(request.body.completed)],
      )
